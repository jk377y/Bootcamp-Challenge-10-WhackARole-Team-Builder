...
function buildManager(managers) {
    let managerHTMLSection = ""
    managerHTMLSection += generateManager(managers[0])
    function generateManager(manager) {
        ...
        <div class="manager">
            <ul>
                ...
                <li class="mainText centered">Office Number: <a href="tel:1"><span class="email">${manager.getOfficeNumber()}</span></a></li>
            </ul>
        </div><br>`
    } return managerHTMLSection;
};

// creating divs for all the engineers
function buildEngineers(engineers) {
    let engineersHTMLSection = ""
    for (i = 0; i < engineers.length; i++) {
        engineersHTMLSection += generateEngineer(engineers[i])
    }
    ...
    }return engineersHTMLSection;
};

// creating divs for all the interns
function buildInterns(interns) {
    let internsHTMLSection = ""
    for (i = 0; i < interns.length; i++) {
        internsHTMLSection += generateIntern(interns[i])
    }
